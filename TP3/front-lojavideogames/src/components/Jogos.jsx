...
function Jogos() {
  ...
  const carregarJogos = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:8082/jogos");
  ...
  const salvarJogo = async (e) => {
    ...
    try {
      ...
      const url = editando
        ? `http://localhost:8082/jogos/${editando.id}`
        : "http://localhost:8082/jogos";
  ...
  const deletarJogo = async (id) => {
    try {
      const res = await fetch(`http://localhost:8082/jogos/${id}`, { method: "DELETE" });
